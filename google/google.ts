...
export const getSheet = async (config: GoogleConfigSheets):Promise<gSheets.sheets_v4.Schema$ValueRange|null> => {
  if(!config.apiKey) {
    return null;
  }
  const jwt = getJwt('spreadsheets');
  return getSheetRequest(jwt, config.apiKey, config.spreadsheetId, config.range);
};

const getSheetRequest = async (jwt: auth.JWT, apiKey: string, spreadsheetId: string, range: string) => {
...
export const getFileRequest = async (jwt: auth.JWT, apiKey: string, fileId: string, fields?: string[]) => {
